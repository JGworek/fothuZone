...
public interface UserDTORepository extends JpaRepository<UserDTO, Integer> {
	...
	@Transactional
	@Query(nativeQuery = true, value = "SELECT * FROM pets.users WHERE NOT EXISTS (SELECT defender_id FROM pets.challenge_requests WHERE (attacker_id = ?1) AND (accepted_status = false AND rejected_status = FALSE)) AND id NOT IN (?1, 2147483647)")
